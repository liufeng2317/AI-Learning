...
可以看到Encoder block包含一个Multi-Head attention，而 Decoder block包含两个 Multi-Head Attention（其中一个用到Masked）。Multi-Head Attention 上方还包含一个 Add&Norm层，Add表示残差连接用于防止网络退化，Norm表示Layer Normalization，用于对每一层的激活值进行归一化。